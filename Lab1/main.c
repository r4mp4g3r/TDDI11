...
{	
	...
	while(true)
	{
        ...
        if(((inputRegister >> 4) & 0x01) == 0) 
			isAcOn = false; // Assignment
		if(((inputRegister >> 4) & 0x01) == 1)
			isAcOn = true;
		...
		//printf("temperatureDesired=%f\n",temperatureDesired); //for debug
		...
		unsigned int temperatureMeasured_fractional = (inputRegister >> 10) & 0x07; // Assignment
		...
		if(temperatureMeasured_fractional == 0x00 && temperatureMeasured_decimal == 0x1F)
			temperatureMeasured = -1;
		else if(temperatureMeasured_fractional == 0x7 && temperatureMeasured_decimal == 0x1F)
			temperatureMeasured = 31;
		//printf("%f",temperatureMeasured);
			// Assignment
			// Assignment
		...
		if(fanSpeedSelector == 0x0)
			...
			fanSpeed = 0x0; // 00=OFF	Assignment
